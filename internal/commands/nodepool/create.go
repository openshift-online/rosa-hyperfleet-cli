...
func runCreate(ctx context.Context, opts *createOptions) error {
	...
	// Auto-discover infra fields from cluster spec if not provided
	if opts.subnetID == "" || opts.instanceProfile == "" || opts.securityGroups == "" {
		...
		if opts.subnetID == "" {
			if subnets, ok := cluster.Spec["privateSubnetIds"].([]interface{}); ok && len(subnets) > 0 {
				opts.subnetID, _ = subnets[0].(string)
			}
		}
		if opts.instanceProfile == "" {
			opts.instanceProfile, _ = cluster.Spec["workerInstanceProfileName"].(string)
		}
		if opts.securityGroups == "" {
			opts.securityGroups, _ = cluster.Spec["workerSecurityGroupId"].(string)
		}
	}
	...
	for i := range sgList {
		sgList[i] = strings.TrimSpace(sgList[i])
	}
